...
public class NescAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
	...
	protected void smartIndentAfterNewLine(IDocument document,
		...
		try {
			...
			if (getBracketCount(document, start, command.offset, true) > 0 &&
					(contentStart >= lineEnd || document.getChar(contentStart) != '}')) {
				...
				//if (command.offset == 0
				//		|| !(computeAnonymousPosition(document, command.offset - 1,
				//				IDocumentExtension3.DEFAULT_PARTITIONING, lineEnd) != -1)) {
					if (lineEnd - contentStart > 0) {
						command.length = lineEnd - command.offset;
						buf.append(document.get(contentStart, lineEnd - contentStart)
								.toCharArray());
					}
				//}
				...
				buf.append('}');
//				int bound = command.offset > 200 ? command.offset - 200 : CHeuristicScanner.UNBOUND;
...
//				}
			...
			// insert extra line upon new line between two braces
				...
				//if (firstCharPos != CHeuristicScanner.NOT_FOUND && document.getChar(firstCharPos) == '{') {
					buf.append("    ");
					command.caretOffset = command.offset + buf.length();
					command.shiftsCaret = false;
