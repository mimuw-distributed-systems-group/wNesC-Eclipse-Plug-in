...
/**
 * Utility methods for heuristic based NesC manipulations in an incomplete NescC
 * source file. Adapted from CDT CHeuristicScanner
 * 
 * <p>
...
public class NescHeuristicScanner implements Symbols {
	...
	private static class NonWhitespace extends StopCondition {
		/*
		 * @see
		 * org.eclipse.cdt.internal.ui.text.CHeuristicScanner.StopCondition#
		 * stop(char)
		 */
		@Override
	 ...
	 *            the partition to scan in
	 */
	public NescHeuristicScanner(IDocument document, String partitioning,
			String partition) {
	...
	/**
	 * Calls
	 * <code>this(document, INCPartitions.NC_PARTITIONING, IDocument.DEFAULT_CONTENT_TYPE)</code>
	 * .
	 * 
	 * @param document
	 *            the document to scan.
	...
	public NescHeuristicScanner(IDocument document) {
		this(document, INCPartitions.NC_PARTITIONING,
	...
	private int getToken(String s) {
		...
		case 7:
			...
			break;
		case 8:
			if ("provides".equals(s))
				return TokenPROVIDES;
			break;
		case 9:
	...
	public int findClosingPeer(int start, int bound, final char openingPeer,
		...
		try {
			...
			start -= 1;
			while (true) {
				start = scanForward(start + 1, bound, new CharacterMatch(
	...
	public int findOpeningPeer(int start, char openingPeer, char closingPeer) {
		return findOpeningPeer(start, NescHeuristicScanner.UNBOUND,
	...
	public IRegion findSurroundingBlock(int offset) {
		...
		int begin = findOpeningPeer(offset - 1, NescHeuristicScanner.UNBOUND,
				LBRACE, RBRACE);
	 ...
	 *         <code>position</code>, <code>bound</code>) that resides in a C
	 ...
	 *         <code>bound</code>, <code>position</code>] that resides in a C
	 *         partition, or <code>NOT_FOUND</code> if none can be found
	 */
	 ...
	 *         <code>position</code>] that resides in a C partition, or
	 *         <code>NOT_FOUND</code> if none can be found
	 */
	...
	/**
	 * Checks whether <code>position</code> resides in a default (C) partition
	...
	public boolean isBracelessBlockStart(int position, int bound) {
		...
		case TokenELSE:
			return true;
		case TokenRPAREN:
			position = findOpeningPeer(fPos, NescHeuristicScanner.UNBOUND,
					LPAREN, RPAREN);
			if (position > 0) {
	 ...
	 * <code>start</code> appears to contain a field reference, i.e. a
	 * (optional) name preceded by a <code>.</code> or <code>-&gt;</code> or
	 * <code>::</code>.
	 * 
	 * @param start
	 *            the position after the field reference operator.
	 * @param bound
	 *            the first position in <code>fDocument</code> to not consider
	 *            any more, with <code>bound</code> &lt; <code>start</code>, or
	 *            <code>UNBOUND</code>
	 * @return <code>true</code> if the current position looks like a field
	 *         reference
	 */
	...
	/**
	 * Returns <code>true</code> if the document, when scanned backwards from
	 * <code>start</code> appears to be a composite type (class, struct, union)
	 * or enum definition. Examples:
	 * 
	 * <pre>
	 * struct A {
	 * enum E {
	 ...
	 *            any more, with <code>bound</code> &lt; <code>start</code>, or
	 *            <code>UNBOUND</code>
	 * @return <code>true</code> if the current position looks like a composite
	 *         type definition
	 */
	public boolean looksLikeCompositeTypeDefinitionBackward(int start, int bound) {
		...
		outerWhile: while (token == Symbols.TokenIDENT) {// type name or base
															// type
			token = previousToken(getPosition(), bound);
			// match nested-name-specifier
			while (token == Symbols.TokenCOLON) { // colon of qualification
				token = previousToken(getPosition(), bound);
				if (token != Symbols.TokenCOLON) { // second colon of
													// qualification
					break outerWhile;
				}
				token = previousToken(getPosition(), bound);
	...
	/**
	 * A simplified interface to NescHeuristicScanner's nextToken() and
	 * previousToken() methods.
	 */
	public static class TokenStream {
		private NescHeuristicScanner fScanner;
		private int fPos;
		private final int fDocumentLength;

		public TokenStream(IDocument document, int startPos) {
			fScanner = new NescHeuristicScanner(document);
			fPos = startPos;
