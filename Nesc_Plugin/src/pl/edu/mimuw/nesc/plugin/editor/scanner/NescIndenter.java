...
import static pl.edu.mimuw.nesc.plugin.editor.scanner.NescHeuristicScanner.NOT_FOUND;
 ...
 * Adapted from CDT
 */
public final class NescIndenter {
	 ...
	 */
	private final NescHeuristicScanner fScanner;
	 ...
	 */
	public NescIndenter(IDocument document, NescHeuristicScanner scanner) {
	 ...
	 */
	public NescIndenter(IDocument document, NescHeuristicScanner scanner, IProject project) {
	...
	public StringBuilder computeContinuationLineIndentation(int offset) throws BadLocationException {
		...
		if (string.trim().isEmpty())
			return reference;
		// Add additional indent
		return createReusingIndent(reference, 4, 0);
	}
	...
	private StringBuilder createIndent(int start, final int indent, final boolean convertSpaceRunsToTabs) {
		final boolean convertTabs= false;
		final int tabLen= 4;
	...
	public StringBuilder createReusingIndent(StringBuilder buffer, int additional, int extraSpaces) {
		...
		int addLength= 4 * additional + extraSpaces; // may be < 0
...
//		}
	...
	public int findReferencePosition(int offset, int nextToken) {
		...
		if (offset < fDocument.getLength()) {
			try {
				...
				int prevToken= fScanner.previousToken(prevPos, NescHeuristicScanner.UNBOUND);
				boolean bracelessBlockStart= fScanner.isBracelessBlockStart(prevPos, NescHeuristicScanner.UNBOUND);
				...
				case Symbols.TokenLBRACE: // for opening-brace-on-new-line style
					if (bracelessBlockStart) {
						extraIndent= 0;//fPrefs.prefIndentBracesForBlocks ? 0 : -1;
	...
	public int findReferencePosition(int offset, boolean danglingElse, MatchMode matchMode) {
		...
		while (fToken == Symbols.TokenCOLON) {
			nextToken();
		}
...
//			}
	...
	private int matchTypeDeclaration() {
		while (true) {
			...
			if (fToken == Symbols.TokenSTRUCT || fToken == Symbols.TokenUNION) {
...
//				}
	...
	private int getBlockIndent(boolean isMethodBody, boolean isTypeBody) {
		if (isTypeBody) {
			return 4; //fPrefs.prefTypeIndent + fPrefs.prefAccessSpecifierIndent;
		} else if (isMethodBody) {
			return 4;//fPrefs.prefMethodBodyIndent + (fPrefs.prefIndentBracesForMethods ? 1 : 0);
		} else {
			return fIndent;
		}
	}
...
//				return false;
	...
	private int handleScopeIntroduction(int bound, boolean firstToken) {
		...
		case Symbols.TokenLPAREN:
			...
			if (looksLikeMethodDecl()) {
				if (true) {//firstToken ? fPrefs.prefMethodDeclFirstParameterDeepIndent : fPrefs.prefMethodDeclDeepIndent) {
					return setFirstElementAlignment(pos, bound);
				} //else {
//					fIndent= 4;//fPrefs.prefMethodDeclIndent;
//					return pos;
//				}
			} else {
				fPosition= pos;
				if (looksLikeMethodCall()) {
					if (true){//firstToken ? fPrefs.prefMethodCallFirstParameterDeepIndent : fPrefs.prefMethodCallDeepIndent) {
						return setFirstElementAlignment(pos, bound);
					} //else {
//						fIndent= 4;
//						return pos;
//					}
				} else if (true) {//fPrefs.prefParenthesisDeepIndent) {
					return setFirstElementAlignment(pos, bound);
				}
			}
...
//				else
				...
				fIndent= 4;//fPrefs.prefNamespaceBodyIndent;
			} else if (looksLikeEnumDeclaration()) {
				fIndent = 4;//fPrefs.prefTypeIndent;
			} else {
				int typeDeclPos = matchTypeDeclaration();
				if (typeDeclPos == NOT_FOUND) {
					fIndent= 4;//fPrefs.prefBlockIndent;
				} else {
					fIndent= 4;// fPrefs.prefAccessSpecifierIndent + fPrefs.prefTypeIndent;
				}
			}
...
//			}
	...
	private boolean skipPointerOperators() {
		if (fToken == Symbols.TokenSTAR || fToken == Symbols.TokenREF) {
			return true;
	...
	private void nextToken(int start) {
		fToken= fScanner.previousToken(start - 1, NescHeuristicScanner.UNBOUND);
		fPreviousPos= start;
	...
	private int peekToken() {
		return fScanner.previousToken(fPosition - 1, NescHeuristicScanner.UNBOUND);
	}
	...
	private boolean looksLikeMethodDecl() {
		...
		case Symbols.TokenIDENT: // method name
			...
			case Symbols.TokenRBRACE:
				...
				return true;
			case Symbols.TokenLBRACE:
				if (fScanner.looksLikeCompositeTypeDefinitionBackward(fPosition, NescHeuristicScanner.UNBOUND)) {
					fPosition= pos;
					return true;
				}
				break;
